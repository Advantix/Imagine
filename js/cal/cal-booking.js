...
(function($) { // create private scope (inside you can use $ instead of jQuery)
	...
	var monthlengths = '31,28,31,30,31,30,31,31,30,31,30,31'.split(',');
  	var dateRegEx = /^([01][1-9]|3[0-1])\/(0?[1-9]|1[0-2])\/(\d{4})$/;
	var yearRegEx = /^\d{4,4}$/;
    ...
    $.fn.simpleDatepicker = function(options) {
		...
		function newDatepickerHTML () {
			...
			var table = jQuery('<table class="datepicker" cellpadding="0" cellspacing="0" style="z-index:100"></table>');
		...
		function closeIt (el, datepicker, dateObj) {
			if (dateObj && dateObj.constructor == Date)
				...
				if (typeof(dateObj) != "undefined"){
					loadtimediv(document.getElementById('booking_date').value,dateObj);
				}
				document.getElementById('selected_time').value=0;
			datepicker.remove();
			datepicker = null;
