...
function getMenuCatList() {
	$.getJSON(serviceMenuURL, function(data) {		
		...
		$.each(cats, function(index, cat) {
			...
			$('#employeeList').append('<li data-theme="c" class="ui-btn ui-btn-icon-right ui-li ui-corner-top ui-btn-up-c"><div class="ui-btn-inner ui-li ui-corner-top"><div class="ui-btn-text"><a href="showMenu.html?catId='+cat.cat_id+'" class="ui-link-inherit" rel="external"><img src="'+itemImgURL+(cat.category_image!=""? cat.category_image:defaultImgURL)+'" style="padding:5px;">&nbsp;' + cat.category_name + '&nbsp;<span style="font-size:11px">('+ cat.category_desc + ')</span></a></div><span class="ui-icon ui-icon-arrow-r"></span></div></li>');
		});
...
function getMenuList() {
	...
	$.getJSON(serviceURL+'itemlist/'+store_id+'/'+catId, function(data) {		
		...
		$.each(items, function(index, item) {
			...
			$('#employeeList').append('<li data-theme="c" class="ui-btn ui-btn-icon-right ui-li ui-corner-top ui-btn-up-c"><div class="ui-btn-inner ui-li ui-corner-top"><div class="ui-btn-text"><a href="#" class="ui-link-inherit" rel="external" onclick="showDealCartAlert('+item.item_id+','+dealFlag+','+dealItemsId+')"><img src="'+itemImgURL+(item.item_img!=""? item.item_img:defaultImgURL)+'" style="padding:5px;">&nbsp;' + item.item_name + '&nbsp;<span style="font-size:11px">('+ item.item_desc + ')</span></a></div><span class="ui-icon ui-icon-arrow-r"></span></div></li>');
		});
		...
		$.each(itemDets, function(index, itemDet) {
			...
			if(itemDet.item_small_price==0 || itemDet.deal_items_id!="") {
				...
				htmlVal+='<div class="ui-grid-b price-cat">';
					htmlVal+='<div class="ui-block-a"><div class="ui-bar ui-bar-f"><h2>Small</h2></div></div>';
					htmlVal+='<div class="ui-block-b"><div class="ui-bar ui-bar-f" ><h2>Large</h2></div></div>';
					htmlVal+='<div class="ui-block-c"><div class="ui-bar ui-bar-f"><h2>Family</h2></div></div>';
				htmlVal+='</div>';
